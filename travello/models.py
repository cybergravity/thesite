...
class Destination(models.Model):
    unique_id=models.CharField(max_length=10,default='123455')
...
class Calculation(models.Model):
    destination = models.OneToOneField(Destination,on_delete=models.DO_NOTHING)
...
def calculate_bill(sender,instance,created,**kwargs):
    if created:
        ...
        try:
            ...
            calculation = Calculation.objects.create(destination=instance,total_amount=total_amount,sgst=sgst,cgst=cgst,total_amount_after_tax=total_amount_after_tax,total_amount_in_words=total_amount_in_words)
        calculation.save()
